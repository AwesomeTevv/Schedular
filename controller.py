...
data = [
    f"data_" + str(i) + ".txt"
    for i in range(config["dataset-sizes"][config["dataset"]])
]
...
print(
    "| {:10} | {:10} | {:8} | {:8} | {:10} | {:7} |".format(
        "Scheduler", "Turnaround", "Response", "Burst", "Interrupt", "Score"
    )
)
print("---------------------------------------------------------------------------")
for scheduler in sorted_marker_data:
    ...
    print(
        "| {:10} | {:10} | {:8} | {:8} | {:10} | {:7} |".format(
            scheduler[0], metrics[0], metrics[1], metrics[2], metrics[3], metrics[4]
        )
    )
